 ...
 * @brief   This is the "Example 10.1 Using a task notification in place of a semaphore,
 *          method 1" from the Mastering the FreeRTOS Real Time Kernel v1.0 book.
 *          
 *          Example 7.1 used a binary semaphore to unblock a task (vHandlerTask()) from
 *          within an interrupt service routine (TIM3 timeout interrupt - HAL_TIM_PeriodElapsedCallback()),
 *          effectively synchronizing the task with the interrupt.
 *          This example replicates the funcionality of example 7.1, but uses a direct to
 *          task notification in place of the binary semaphore.
 * 
...
static void vHandlerTask( void *pvParameters )
{
  ...
  /* will store the number of pending events returned from ulTaskNotifyTake() */
  ...
  for ( ;; )
  {
    ...
    {
      /* To get here at least one event must have ocurred.
         Loop here until all the pending events have been processed
         (in this case, just print out a message for each event) */
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
  ...
  if ( htim->Instance == TIM3 )
  {
    ...
    vTaskNotifyGiveFromISR( xHandlerTask, /* handle of the task to which the notification is being sent.
                                             The handle was saved when the task was created */
