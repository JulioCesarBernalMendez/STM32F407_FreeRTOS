 ...
 * @brief   This is the "Example 4.7 Definining an Idle task hook function" from the Mastering the FreeRTOS Real
 *          Time Kernel v1.0 document.
 * 
 *          Use of blocking vTaskDelay() API in previous Example 4.4 created a lot of idle time, that is, time when
 *          the Idle task executes because both application tasks are in Blocked state. Example 4.7 makes use of this
 *          idle time through the addition of an Idle hook function.
 * 
 *          The function that implements the created tasks (vTaskFunction()) is modified slightly to print out a
 *          counter value which is icremented via the Idle Hook function (vApplicationIdleHook()).
 * 
 *          The number of iterations between the application tasks depends on the hardware speed.
 * 
...
/* ulIdleCycleCount in string format to be printed out by the tasks */
static char ulIdleCycleCountString[ 32 ];
/* USER CODE END PV */
