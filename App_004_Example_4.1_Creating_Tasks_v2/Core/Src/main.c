 ...
 * @brief   This is the "Example 4.1 Creating tasks v2" from the Mastering the FreeRTOS Real Time Kernel v1.0 document,
 *          unlike v1, here task2 is created within task1 instead.
...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define mainDELAY_LOOP_COUNT    (5000)
/* USER CODE END PD */
...
/* USER CODE BEGIN PFP */
void UART2_Print_Text( UART_HandleTypeDef *huart, const char *text, uint8_t size );
...
int main(void)
{
  ...
  /* Create one of the two tasks.
     ...
     to ensure the task was created successfully */
...
void UART2_Print_Text( UART_HandleTypeDef *huart, const char *text, uint8_t size )
{
  HAL_UART_Transmit( huart, ( const uint8_t* ) text, size, 5000 );
}
...
void vTask1( void *pvParameters )
{
  ...
  /* string to print out when task1 executes */
  const char *pcTaskName = "Task1 is running\n\r";

  /* if this task code is executing then the scheduler must already have been started.
     Create another task before entering the infinite loop */

  /* create the second task in exactly the same way and at the same priority */
  xTaskCreate( vTask2, "Task 2", 128, NULL, 1, NULL );

  for ( ; ; )
  {
    /* print out the name of this task (task1) */
    UART2_Print_Text( &huart2, pcTaskName, 18 );
    ...
    for ( ulCount = 0; ulCount < mainDELAY_LOOP_COUNT; ulCount++ )
    {
      /* This loop is just a very crude delay implementation.
         ...
         Later examples will replace this loop with a proper delay/sleep function */
    }
  }
}
...
void vTask2( void *pvParameters )
{
  ...
  for ( ; ; )
  {
    /* print out the name of this task (task2) */
    UART2_Print_Text( &huart2, "Task2 is running\n\r", 18 );
