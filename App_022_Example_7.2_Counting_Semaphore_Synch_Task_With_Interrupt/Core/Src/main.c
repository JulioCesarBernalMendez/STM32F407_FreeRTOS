 ...
 * @brief   This is the "Example 7.2 Using a counting semaphore to synchronize a task with an interrupt"
 *          from the Mastering the FreeRTOS Real Time Kernel v1.0 book.
 *          
 *          This example improves on the example 7.1 implementation by using a counting semaphore
 *          in place of the binary semaphore. main() is changed to include a call to
 *          xSemaphoreCreateCounting() in place of the call to xSemaphoreCreateBinary.
 *
 *          To simulate multiple events ocurring at high frequency, the TIM3 timeput interrupt callback
 *          (HAL_TIM_PeriodElapsedCallback()) is changed to give the semaphore more than once per
 *          interrupt. Each event is latched in the semaphore's count value.
 * 
 *          All the other functions remain unmodified from those used in example 7.1.
 * 
 *          The task to which interrupt handling is deferred, processes all three events
 *          (one generated via the TIM3 interrupt, the other two simulated via the counting semaphore)
 *          each time an interrupt is generated. The events are latched into the count value of the
 *          semaphore, allowing the task (vHandlerTask()) to process them in turn.
 * 
