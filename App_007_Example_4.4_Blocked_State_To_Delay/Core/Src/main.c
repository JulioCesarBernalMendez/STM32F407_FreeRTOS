 ...
 * @brief   This is the "Example 4.4 Using the Blocked state to create a delay" from the Mastering the FreeRTOS Real
 *          Time Kernel v1.0 document.
 * 
 *          Previous examples were periodic (they have delayed for a 'period' then printed out their string), before
 *          delaying more, and so on. The delay was generated crudely using a NULL loop. Example 4.3 demonstrated the
 *          disadvantage of this method (NULL loop), the higher priority task remained in Running state while executing
 *          the NULL loop, 'starving' the lower priority task of any processing time, another disadvante is it uses the
 *          maximum processing time even though it does not have any work to do.
 *          Example 4.4 corrects the behavior explained above by replacing the NULL loop with a call to vTaskDelay(),
 *          which places the calling task into Blocked state for a fixed number of tick interrupts, plus the task does
 *          not waste any processing time while in Blocked state, only uses processing time when it has work to do.
 *          Even though the 2 tasks are still (Example 4.3) created at different priorities, both will now run.
 * 
...
/* USER CODE BEGIN PV */
/* Define the strings that will be passed in as the task parameter.
   These are defined const and not on the stack used by main() to ensure
